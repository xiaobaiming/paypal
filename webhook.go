...
func (this *PayPal) GetInvoiceNumber(body []byte) (invoiceNumber string) {
	if len(body) == 0 {
		return ""
	}
	...
	if event.Sale().InvoiceNumber == ""{
		return ""
	}
	invoiceNumber =event.Sale().InvoiceNumber

	return invoiceNumber
}

// GetWebhookEvent 用于处理 webbook 回调
func (this *PayPal) GetWebhookEvent(webhookId string, body []byte, head map[string]string) (event *Event, err error) {
	...
	verifyParam.AuthAlgo = head["Paypal-Auth-Algo"]
	verifyParam.CertURL = head["Paypal-Cert-Url"]
	verifyParam.TransmissionId = head["Paypal-Transmission-Id"]
	verifyParam.TransmissionSig = head["Paypal-Transmission-Sig"]
	verifyParam.TransmissionTime = head["Paypal-Transmission-Time"]
